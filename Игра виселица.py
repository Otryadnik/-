...
word_list = ['почта', 'домино', 'декор', 'сигнал', 'техника', 'проверка', 'математика', 'география', 'год', 'человек',
             ...
             'тень', 'апрель', 'коллега', 'преступление', 'рыба', 'кресло', 'запах', 'выставка', 'князь', 'фотография',
             'весна', 'помещение', 'эпоха', 'занятие', 'произведение', 'концерт', 'ладонь', 'дама', 'сомнение',
             'американец', 'середина', 'зарплата', 'тайна', 'запад', 'июнь', 'беседа', 'фронт', 'поезд', 'должность',
             'баба', 'промышленность', 'музей', 'судья', 'получение', 'полковник', 'зритель', 'секретарь', 'установка',
             ...
             'обязанность', 'шея', 'крыша', 'появление', 'учреждение', 'признак', 'труба', 'жертва', 'беда', 'фон',
             'организм', 'ученик', 'заключение', 'выполнение', 'канал', 'исключение', 'дача', 'соглашение', 'осень',
             'польза', 'стул', 'июль', 'дождь', 'сутки', 'еврей', 'конкурс', 'открытие', 'телевизор', 'лошадь',
             'температура', 'приказ', 'лестница', 'реклама', 'спор', 'подруга', 'угроза', 'конфликт', 'изучение',
             'вино', 'концепция', 'достижение', 'сообщение', 'объединение', 'обстановка', 'костюм', 'ключ', 'ресторан',
             'назначение', 'царь', 'воспоминание', 'увеличение', 'вкус', 'мероприятие', 'лоб', 'слой', 'восток',
             'последствие', 'принятие', 'сотрудничество', 'нефть', 'слух', 'бок', 'переговоры', 'тюрьма', 'кандидат',
             'просьба', 'реальность', 'подарок', 'категория', 'потребность', 'быль', 'редакция', 'очко', 'километр',
             'губернатор', 'новость', 'инструмент', 'потеря', 'взаимодействие', 'звонок', 'кусок', 'капитал', 'грех',
             ...
             'сезон', 'издание', 'человечество', 'снижение', 'запас', 'крик', 'публика', 'вещество', 'экран', 'эффект',
             'ящик', 'ракета', 'водитель', 'пакет', 'зеркало', 'вес', 'дно', 'вагон', 'убийство', 'тон', 'щека',
             'дурак', 'длина', 'давление', 'двигатель', 'камера', 'обращение', 'формула', 'запись', 'крыло', 'поездка',
             'гостиница', 'колесо', 'разрешение', 'торговля', 'академия', 'доклад', 'общение', 'присутствие',
             'процедура', 'испытание', 'нож', 'проверка', 'коммунист', 'цифра', 'полет', 'стакан', 'эффективность',
             'обучение', 'портрет', 'достоинство', 'рассмотрение', 'владелец', 'жилье', 'компьютер', 'корень', 'смена',
             ...
             'доска', 'ожидание', 'платье', 'смех', 'сбор', 'пенсия', 'буква', 'порог', 'автобус', 'воспитание',
             'производитель', 'полоса', 'риск', 'пиво', 'корпус', 'штаб', 'кольцо', 'постель', 'выпуск', 'дворец',
             'брак', 'прокурор', 'печать', 'окончание', 'автомат', 'тенденция', 'следователь', 'штат', 'куст',
             ...
             'термин', 'прибыль', 'стандарт', 'восторг', 'гибель', 'изделие', 'вооружение', 'осуществление', 'уход',
...
def Check_symbol(possible_answer, tries):
    while possible_answer.lower() not in rus_lower_alphabet:
        possible_answer = input('Неверный ввод. Введите букву русского алфавита: ')

    if tries == 6:
        guessed_letters.append(possible_answer.lower())
    else:
        while possible_answer.lower() in guessed_letters:
            possible_answer = input('Вы уже использовали эту букву. Введите другую: ')
        guessed_letters.append(possible_answer.lower())
...
def play(word):
    ...
    symbol_in_word = []
    symbol_word_completion = []

    symbol_word_completion = list(word_completion)
    symbol_in_word = list(word)

    while '_' in word_completion and tries != 0:
        print(word_completion)
        possible_answer = input('Введите букву или слово целиком: ')

        if len(possible_answer) > 1:
            if possible_answer == word:
                word_completion = word
                return 'Поздровляем! Вы угодлали. Это слово' + word_completion
            else:
                if tries == 6:
                    guessed_words.append(possible_answer.lower())
                    print('Не верно. Попробуй ещё раз.')
                    tries -= 1
                    print(display_hangman(tries))
                else:
                    if possible_answer.lower() in guessed_words:
            ...
            possible_answer = Check_symbol(possible_answer, tries)
            if possible_answer not in word:
                print('Не верно. Попробуй ещё раз.')
                tries -= 1
                print(display_hangman(tries))

            else:
                for c in range(len(symbol_in_word)):
                    if symbol_in_word[c] == possible_answer:
                        symbol_word_completion[c] = symbol_in_word[c]
                        word_completion = ''.join(symbol_word_completion)
                    else:
                        continue
        if tries == 0:
            return 'Вы проиграли. Загаданое слово:' + word
    return 'Поздровляем! Вы угодлали. Это слово' + word_completion
...
finish = 'д'
while finish.lower() == 'д':
    finish = input('Хотите сыграть ещё раз. (да = д,  нет = н): ')
    if finish == 'д':
        print(play(get_word()))
    else:
